...
export default function DeleteSaleModal({
  ...
  const [deleteOption, setDeleteOption] = useState<any>(
    currentLanguage === 'en-EN' ? 'Delete from Accounting' : 'Muhasebeden Sil'
  );
  const { handleSubmit } = useForm();
  const { isPending: isDeleting, mutateAsync: deleteSales } = useDeleteSales();
        ...
        <form onSubmit={handleSubmit(onSubmit)}>
          <StyledBox>
            ...
            {currentLanguage === 'en-EN' && (
              ...
              >
                Deleted sales cannot be brought back, are you sure you want to
                delete
                <StyledSpan>
                  {formatTimestampToDate(row.saleCreatedAt)}
                </StyledSpan>{' '}
                dated sale to
                <StyledSpan>{row.saleCompanyName} ?</StyledSpan> The delete
                operation will also delete the sales record from the accounting
                table by default. If you do not want it to be deleted, you can
            ...
            {currentLanguage === 'tr-TR' && (
              <Typography
                ...
                sx={{ margin: '1.3rem 0', fontSize: '1.4rem' }}
              >
                Silinen satış bilgileri geri getirilemez ve muhasebe
                kayıtlarınızdan da silinir.
                <StyledSpan>
                  {formatTimestampToDate(row.saleCreatedAt)}
                </StyledSpan>{' '}
                tarihli
                <StyledSpan>{row.saleCompanyName}</StyledSpan> şirketine yapılan
                satışı silmek istediğinize emin misiniz? Silme işleminin satış
                kaydını muhasebe tablosundan da silmesini istemiyorsanız
